...
const GRAM_LABELS: { cond: (g: number, m: number, b: number) => boolean; label: string }[] = [
  { cond: (g, m, b) => b > 0.04 && m > 0.06,  label: "Yes/no question — raised brows + open mouth" },
  { cond: (g, m, b) => b < -0.04 && m > 0.06, label: "Wh-question — furrowed brows + open mouth" },
  { cond: (g, m, b) => b < -0.04 && m < 0.02, label: "Negation — furrowed brows, closed mouth" },
  { cond: (g, m, b) => b > 0.05 && m < 0.02,  label: "Intensifier — raised brows, closed mouth" },
  { cond: (g, m, b) => Math.abs(g) > 0.08,    label: "Referential shift — gaze toward locus" },
  { cond: () => true,                           label: "Neutral / declarative" },
];

function LiveFaceViz({ uN }: LiveFaceProps) {
  // u^N layout: [gaze_x(0), gaze_y(1), mouth(2), brow(3), head_tilt(4)]
  const gazeX  = uN ? uN[0] * 6 : 0;
  const mouth  = uN ? Math.min(1, Math.max(0, uN[2] * 8)) : 0;
  const brow   = uN ? uN[3] * 5 : 0;

  const gramLabel = GRAM_LABELS.find(({ cond }) => cond(
    uN ? uN[0] : 0, uN ? uN[2] : 0, uN ? uN[3] : 0
  ))!.label;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
          {uN ? [
            { label: "gaze_x", val: uN[0], color: "var(--sky)" },
            { label: "gaze_y", val: uN[1], color: "var(--sky)" },
            { label: "mouth",  val: uN[2], color: "var(--coral)" },
            { label: "brow",   val: uN[3], color: "var(--mint)" },
            { label: "tilt",   val: uN[4], color: "var(--lav)" },
          ].map(({ label, val, color }) => (
